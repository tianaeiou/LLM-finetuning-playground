...
def get_example_prompt(cur_neighbor_dict, num_examples, data_index, ref_df, min_value=0, max_value=1,
                       is_regression_task=True, example_template=EXAMPLE_TEMPLATE):
...
def create_prompt_for_single_row(row, example_prompt, instruction_description, context_description, predict_target_name,
                                 ...
                                 cls_template=PROMPT_TEMPLATE_CLS):
    ...
    if is_regression_task:
        template = reg_template_with_example if example_prompt else reg_template
        ...
        template = cls_template_with_example if example_prompt else cls_template
...
def generate_hybrid_instruction(data_df, ref_df, instruction_description, context_description, predict_target_name,
    ...
    for idx, row in data_df.iterrows():
        ...
        cur_prompt = create_prompt_for_single_row(row, example_prompt, instruction_description, context_description,
                                                  ...
                                                  label1_description=label1_description)
        prompts_list.append(cur_prompt)
    return prompts_list


def process_dataset(data_df, ref_df, dataset_name, neighbor_info_dir, k, use_hybrid):
    """
...
group = admet_group(path='data/')
for dataset in group.dataset_names:
    benchmark = group.get(dataset)
    train_val_df, test_df = benchmark['train_val'], benchmark['test']
    if mode == "train":
        ref_df = train_val_df
...
# with open(f"{project_path}/instruction_tuning/instructions/regression_all-0-shot_new.json", 'w') as json_file:
#     json.dump(regression_task_prompt_all, json_file)
